...
${address.phone}
 ...
 document.getElementById('addressContainer').addEventListener('click',async(e)=>{
...
const getMyOrders = async () => {
    ...
    container.innerHTML = '<p class="section-title">Order History</p>'
    
    if (!result.order || result.order.length === 0) {
        container.innerHTML += `<p style="color:var(--muted)">No orders yet.</p>`
        return
    }

    result.order.forEach(order => {
        const firstItem = order.items[0]
        const image = firstItem.image && firstItem.image[0]
            ? firstItem.image[0]
            : 'placeholder.jpeg'
