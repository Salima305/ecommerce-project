...
.addEventListener("click", async () => {
    ...
    const correctTotal = result.finalTotal + shipping
        document.getElementById("total").textContent =`₹${correctTotal.toLocaleString()}`
        msg.textContent = `Coupon applied — ₹${Math.round(result.discountAmount)} off`
        document.getElementById("discount").textContent = `-₹${Math.round(result.discountAmount).toLocaleString()}`
